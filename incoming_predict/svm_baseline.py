...
train['income']  = train['income'].map(lambda item : 1 if item == '>50K' else 0 ) # >50K is 1 and <=50K is 0 
...
# predict
predict_label = pd.DataFrame(classifier.predict(predict_data).reshape((predict_data.shape[0], 1)), columns=['income'])
