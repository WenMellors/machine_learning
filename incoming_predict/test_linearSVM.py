...
train_x, eval_x, train_y, eval_y = train_test_split(train_data, train_label, test_size=0.2)
clf = LinearSVM(max_iter=10000, verbose=1000, learning_rate = 0.001)
clf.fit(np.array(train_x).reshape(train_x.shape[0], train_x.shape[1]), np.array(train_y).reshape(train_y.shape[0], 1))
eval_result = clf.predict(np.array(eval_x))

# test accuracy 
print('ac ', accuracy_score(eval_y, eval_result))
print('precision ', precision_score(eval_y, eval_result))
print('recall ', recall_score(eval_y, eval_result))
print('f1_score ', f1_score(eval_y, eval_result))

# predict
predict_label = pd.DataFrame(clf.predict(np.array(predict_data)).reshape((predict_data.shape[0], 1)), columns=['income'])
